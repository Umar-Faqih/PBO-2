...
class FrmBuku:
    ...
    def aturKomponen(self):
        ...
        Label(mainFrame, text='JUDUL BUKU:').grid(row=1, column=0,
            sticky=W, padx=5, pady=5)
        Label(mainFrame, text='PENULIS BUKU:').grid(row=2, column=0,
            sticky=W, padx=5, pady=5)
        Label(mainFrame, text='PENERBIT BUKU:').grid(row=3, column=0,
            sticky=W, padx=5, pady=5)
        Label(mainFrame, text='TAHUN TERBIT:').grid(row=4, column=0,
            sticky=W, padx=5, pady=5)
        Label(mainFrame, text='GENRE:').grid(row=5, column=0,
            sticky=W, padx=5, pady=5)
        # Textbox
        ...
        # Combo Box
        self.txtGenre = StringVar()
        Cbo_genre = ttk.Combobox(mainFrame, width=37, textvariable = self.txtGenre) 
        Cbo_genre.grid(row=5, column=1, padx=5, pady=5)
        # Adding genre combobox drop down list
        Cbo_genre['values'] = ('komedi','pengetahuan','fantasi','sains')
        Cbo_genre.current()
        # Button
        ...
        # define columns
        columns = ('id_buku','kode_buku','judul_buku','penulis_buku','penerbit_buku','tahun_terbit','genre')
        self.tree = ttk.Treeview(mainFrame, columns=columns, show='headings')
        # define headings
        self.tree.heading('id_buku', text='ID')
        self.tree.column('id_buku', width="30")
        self.tree.heading('kode_buku', text='KODE')
        self.tree.column('kode_buku', width="50")
        self.tree.heading('judul_buku', text='JUDUL BUKU')
        self.tree.column('judul_buku', width="150")
        self.tree.heading('penulis_buku', text='PENULIS BUKU')
        self.tree.column('penulis_buku', width="150")
        self.tree.heading('penerbit_buku', text='PENERBIT BUKU')
        self.tree.column('penerbit_buku', width="130")
    ...
    def onReload(self, event=None):
        ...
        for i, d in enumerate(parsed_data):
            self.tree.insert("", i, text="Item {}".format(i), values=(d["id_buku"],d["kode_buku"],d["judul_buku"],d["penulis_buku"],d["penerbit_buku"],d["tahun_terbit"],d["genre"]))
