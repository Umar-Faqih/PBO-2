...
class FrmPetugas:
    ...
    def aturKomponen(self):
        ...
        Label(mainFrame, text='KODE PETUGAS:').grid(row=0, column=0,
            sticky=W, padx=5, pady=5)
        Label(mainFrame, text='NAMA PETUGAS:').grid(row=1, column=0,
            sticky=W, padx=5, pady=5)
        Label(mainFrame, text='JABATAN PETUGAS:').grid(row=2, column=0,
            sticky=W, padx=5, pady=5)
        # Textbox
        ...
        # Textbox
        self.txtNama_petugas = Entry(mainFrame, width=50) 
        self.txtNama_petugas.grid(row=1, column=1, padx=5, pady=5)
        # Combo Box
        self.txtJabatan_petugas = StringVar()
        Cbo_jabatan_petugas = ttk.Combobox(mainFrame, width = 47, textvariable = self.txtJabatan_petugas) 
        Cbo_jabatan_petugas.grid(row=2, column=1, padx=5, pady=5)
        # Adding jabatan_petugas combobox drop down list
        Cbo_jabatan_petugas['values'] = ('Admin','Kepala Perpus','Ob','Kasir')
        ...
        # define headings
        self.tree.heading('id_petugas', text='ID')
        self.tree.column('id_petugas', width="30")
        self.tree.heading('kode_petugas', text='KODE')
        self.tree.column('kode_petugas', width="80")
        self.tree.heading('nama_petugas', text='NAMA PETUGAS')
        self.tree.column('nama_petugas', width="200")
        self.tree.heading('jabatan_petugas', text='JABATAN PETUGAS')
        self.tree.column('jabatan_petugas', width="200")
        # set tree position
